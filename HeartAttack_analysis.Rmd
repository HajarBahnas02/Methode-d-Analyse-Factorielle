---
title: "Analyse ACP complète du dataset HeartAttack.ridk"
author: "ChatGPT (génération automatique à partir du rapport)"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
# 1. Introduction
Ce notebook reproduit intégralement les étapes décrites dans le rapport du projet sur l’analyse de risque cardiaque à partir du dataset **HeartAttack.ridk**. 
L’objectif est d’exécuter une **analyse en composantes principales (ACP)**, puis d’explorer les résultats à travers des visualisations et des modèles simples.
...
# 9. Conclusion
Ce notebook **HeartAttack.ridk** permet de réaliser un pipeline complet d’analyse :
1. Prétraitement et normalisation des données.
2. Analyse en composantes principales (ACP) avec visualisation.
3. Enrichissement du jeu de données avec les dimensions principales.
4. Exploration de la structure via **k-means** et **régression logistique**.

Les fichiers exportés contiennent :
- `data_final.csv` (dataset enrichi)
- `pca_eigenvalues.csv` (valeurs propres)
- `scree_plot.png`, `correlation_circle_dim1_dim2.png` (visualisations)
